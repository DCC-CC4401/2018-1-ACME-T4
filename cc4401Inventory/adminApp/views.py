    ...
    try:
        if request.method == "GET":
            if request.GET["filter"] == 'vigentes':
                ...
                loans = Loan.objects.filter(ending_date_time__lt=actual_date, article__state='P').order_by(
                    'starting_date_time')
            elif request.GET["filter"] == 'perdidos':
                loans = Loan.objects.filter(ending_date_time__lt=actual_date, article__state='L').order_by(
                    'starting_date_time')
            else:
